...
// and persist the OAuth token set.
export async function upsertUser(info, tokenSet) {
  const isAdmin = adminSet.has(info.id)
  const user = await one(
    `INSERT INTO users (id, username, global_name, avatar, email, is_admin, last_seen, membership_checked_at)
     ...
     RETURNING *`,
    [info.id, info.username, info.global_name ?? null, info.avatar ?? null, info.email ?? null, isAdmin],
  )
    ...
    await query(
      `INSERT INTO oauth_tokens (user_id, refresh_token, scope, access_expires_at, updated_at)
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         updated_at = now()`,
      [info.id, tokenSet.refresh_token, tokenSet.scope || config.oauth.scope, expiresAt],
    )
  }
  return user
}
