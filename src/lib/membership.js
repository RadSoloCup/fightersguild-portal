    ...
    try {
      ...
      if (ok) {
        ...
        if (profile) {
          await query(
            `UPDATE users SET username = $2, global_name = $3, avatar = $4,
               email = COALESCE($5, email), membership_checked_at = now() WHERE id = $1`,
            [row.user_id, profile.username, profile.global_name ?? null,
             profile.avatar ?? null, profile.email ?? null],
          )
        } else {
        ...
        await query(
          ...
          [row.user_id, tok.refresh_token, String(tok.expires_in || 3600)],
        )
      } else {
